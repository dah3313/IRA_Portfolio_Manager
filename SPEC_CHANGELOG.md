...
clamped by two independent ceilings (the existing 7.5% portfolio-%
ceiling AND a new dollar ceiling indexed annually from 2027 at the
canonical `inflation_rate`); the actual payment is the MIN of the
schedule and both ceilings. The dollar ceiling closes a gap in the
post-D-RY-14 design: a high-portfolio Phase 3 latch (~$2M+) would
otherwise authorize an `I_0` far above any plausible living-expense
envelope, and the portfolio-% ceiling alone scales too loosely to
prevent the drain. See DECISIONS.md D-SPEC-9 for the full design
discussion.

(1) §4.1.1.2 rewritten to specify both ceilings, the MIN rule, and
the `monthly_payment_ceiling_bound` alert's responsibility to
identify which ceiling bound; sample-value table added showing
indexed dollar-ceiling values at default parameters across the
Phase 3 horizon ($4,000 in 2027 → $11,210 in 2057). (2) Two new
parameters in §2.3 examples and `ruleset.yaml` §6:
`phase3_dollar_ceiling_base_dollars = 4000` and
`phase3_dollar_ceiling_base_year = 2027`. (3) §13.6 test suite
extended to cover dollar-ceiling-binding and both-ceilings-
simultaneous cases (and the existing portfolio-%-ceiling-binding
case made explicit). (4) §14.6 (Phase 3 re-validation open question)
rewritten around the spread of trigger portfolios spanning the
regimes where each ceiling dominates: small ($300K–$500K) where
`I_0` is the binding constraint, mid ($500K–$1.5M) where neither
ceiling binds, large ($2M+) where the dollar ceiling binds at or
near latch.

Cleanup pass: D-RY-14 (v1.4 Phase 3 design overhaul) had left
substantial dangling references throughout the spec — parameter
names that no longer exist (`FLOOR_2026`, `CEILING_2026`,
`INFLATION_PRE`, `CPI_POST`, `INFLATION_TERMINAL`, `TARGET_TERMINAL`,
`SUB_FLOOR_RATE`, `RETURN_NOM`, `HORIZON`), section references to
nonexistent subsections (`§4.1.1.6`, `§4.1.1.7`, `§4.1.1.8`, `§12.7`),
and entire vocabulary clusters from the deleted design (the
four-regime taxonomy, `sub_floor_regime`, `floor_T`, `ceiling_T`,
`sustainable_floor_monthly`, `I_sustainable`). All purged. The
§13.6 test section, previously specifying "Phase 3 four-regime
testing" against a taxonomy that no longer existed, is rewritten
to test the actual §4.1.1 mechanism. The §14.1 PHASE_3_DESIGN.md
reconciliation entry — historical reconciliation residue with no
current-coding value — is deleted entirely; §14.2–§14.7 renumbered
to §14.1–§14.6 with all 8 affected cross-references updated.

Other historical residue removed: a "F6 case from the threat
model" parenthetical in §15.5 (referring to a threat-model
enumeration that doesn't exist in IRAPM), an "Item 20" review-
checklist artifact at the end of §12.6, two `§12.7` pointers in
§7.2.1 and §8.2.5 (the target content lives in §12.6, not a
nonexistent §12.7), a `(v1.4): the Phase 3 transition is...`
historical-change note in §10.6.3 rewritten as current rule, a
`formerly slave_wake_staleness_days = 3` retrospective in §9.4,
and a "previously specified in days" change-history paragraph in
§9.4 (current rule stated without the historical contrast).

`ruleset.yaml` updates: §6 section header rewritten to describe
both ceilings; existing `phase3_monthly_payment_ceiling_rate`
comment rewritten to reflect its role as one of two clamps (the
"I_0 calibrated against temporarily-inflated portfolio at latch"
bullet, which the new dollar ceiling now covers, is removed); two
new parameters added with full operator-comment rationale. No
code changes in this release.

**Verify-on-your-side note:** the `monthly_payment_ceiling_bound`
alert template gains a `binding_ceiling` placeholder (values:
`portfolio_percent`, `dollar`, or `both`). The alert dispatch
code must populate it for the alert body to render correctly;
until populated, the literal `{binding_ceiling}` text will appear
in dispatched messages. The Phase 3 cycle code that computes the
clamp also needs to compute and pass this value. Both code-side
changes are pending; spec and config files are updated.

Pending code work tracked in DECISIONS.md D-SPEC-9: `ruleset_model.py`
adds the two new fields with `Field(gt=0)` on the dollar base and
a sensible year-range constraint; the Phase 3 cycle code computes
the dollar ceiling each cycle from current year and clamps via
three-way MIN; `alert_templates.yaml` `monthly_payment_ceiling_bound`
template gains the `binding_ceiling` placeholder.
